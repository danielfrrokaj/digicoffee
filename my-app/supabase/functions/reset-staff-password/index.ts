...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (targetProfile.venue_id !== callerProfile.venue_id) {
      return new Response(
        JSON.stringify({ error: 'Not authorized: can only reset passwords for staff from your venue' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    
    // Check if target is a bartender (managers can't reset passwords for other managers)
    if (targetProfile.role !== 'bartender') {
      return new Response(
        JSON.stringify({ error: 'Not authorized: can only reset passwords for bartenders' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
